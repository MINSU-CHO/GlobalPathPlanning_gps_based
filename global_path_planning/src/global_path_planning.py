...
class GlobalPathPlanning:
    def __init__(self):
        ...
        # with open(self.mapdata_path + 'waypoint2.txt') as f:
        #     self.waypoint = f.readlines()
        f = open(self.mapdata_path + 'test6.csv','r')
    ...
    def findMyLane(self):
        for lane in self.lanelist:
            ...
            ymin = float(str[3])
            ymax = float(str[4])
            if self.check(xmin,xmax,ymin,ymax,0.0):
                if name == 'U01':
                    # print("get")
                    if self.state.z < 34:
                        name = 'V72'
                    elif self.check(938.155, 1063.66, 154.483, 178.104,0.0):
                        name = 'H16'
                    else:
                        return False
    ...
    def findCrossSection(self):
        for cross in self.crosslist:
            ...
            ymin = float(str[3])
            ymax = float(str[4])
            if self.check(xmin,xmax,ymin,ymax,0.2):
    ...
    def trajectory_planning(self):
        ...
        for point in self.waypoint:
            if point[0] != '#':
                self.state.x = (float(point[3]) - 126.89*np.pi/180) * 6371 * 1000 * math.cos(float(point[2]))
                self.state.y = (float(point[2]) - 37.58*np.pi/180) * 6371 * 1000
                self.state.z = float(point[1])
                if self.findMyLane():
                    temp_list.append(self.currentLane[:3])
                elif self.findCrossSection():
                    temp_list.append(self.currentLane)
        ...
        for i, lane in enumerate(self.globalpath):
            ...
            if i < len(self.globalpath)-1:
                ...
                if lane[0] != 'C' and next_lane[0] != 'C':
                    ...
                    elif self.lanelist[idx].split()[6] == self.lanelist[idx2].split()[6]:
                        cross = self.lanelist[idx].split()[6]
                    ...
                    tmp.append(cross)
                elif lane[0] == 'C' and next_lane[0] == 'C':
                    for lane_ in self.lanelist:
                        if lane == lane_.split()[5] and next_lane == lane_.split()[6]:
                            tmp.append(lane_.split()[0])
                        elif lane == lane_.split()[6] and next_lane == lane_.split()[5]:
                            tmp.append(lane_.split()[0])
        ...
        # for idx, lane in enumerate(self.globalpath):
        #     if lane == 'V72':
        #         if idx != len(self.globalpath)-1:
        #             self.globalpath.remove(lane)
        #         else:
